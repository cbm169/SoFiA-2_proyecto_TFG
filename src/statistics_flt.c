...
///                                                                      ///
/// SoFiA 2.1.1 (statistics_flt.c) - Source Finding Application          ///
...
float nth_element_flt(float *data, const size_t size, const size_t n)
{
	...
	while(l < m)
	{ //ordena los valores segun sean más pequeños o mayores que el valor central inicial
		float value = *ptr; //Valor del dato central
		float *i = l; //puntero al inicio
		float *j = m; //puntero al final
		
		do
		{
			while(*i < value) ++i; //busca un valor en la primera mitad, superior al valor central
			while(value < *j) --j; //busca un valor en la segunda mitad, inferior al valor central
			
			if(i <= j) //si no llegaste a la mitad
			{
				float tmp = *i;
				*i = *j;
				*j = tmp;
		...
		} while(i <= j); //mientras no llegues a la mitad
		
		//actualiza el rango de datos: si hay mas valores menores que value desecha los mayores y value, y viceversa
		if(j < ptr) l = i;
		if(ptr < i) m = j;
	}
...
{
	const float value = nth_element_flt(data, size, size / 2); //Devuelve el elemento n menor en los datos
	return IS_ODD(size) || fast ? value : (value + max_flt(data, size / 2)) / 2.0; //si el tamaño es par, devuelve la media value y el anterior valor menor
}
...
//   this will modify the original data array. NaN values    /S/
...
void filter_boxcar_1d_flt(float *data, float *data_copy, const size_t size, const size_t filter_radius)
{
	...
	data[size - 1] = 0.0;
	for(i = filter_size; i--;) data[size - 1] += data_copy[size + i - 1]; //data[size - 1 - filter_radius + i] i = 1:filter_radius*2 ::: data[size - 1 - filter_radius + 1] a data[size - 1 + filter_radius]
